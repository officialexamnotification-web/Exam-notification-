    ...
    <div className={`bg-white border text-gray-800 border-gray-200 rounded shadow-sm flex flex-col ${isFullHeight ? 'h-auto' : 'h-[400px] overflow-hidden'}`}>
      ...
      <ul className={`divide-none px-1 py-1 ${isFullHeight ? 'flex-1' : 'flex-1 overflow-y-auto'}`}>
        ...
        {category.links.map((link) => (
          <li key={link.id} className="group relative border-b border-dashed border-gray-200 last:border-0 border-opacity-70">
            <a href={link.url} className="flex px-2 py-2.5 hover:bg-blue-50 active:bg-blue-100 active:scale-[0.98] transition-all duration-75 items-start gap-1">
              ...
              <div className="flex-1">
                <span className="text-[#104ba6] group-hover:text-blue-800 font-semibold text-[14.5px] leading-snug inline">
                  {link.title}
                  {link.isNew && (
                    <span className="inline-flex ml-1.5 items-center justify-center bg-red-600 text-white text-[11px] uppercase font-black px-1.5 py-[1px] rounded-[3px] animate-pulse whitespace-nowrap align-baseline translate-y-[-1px]">
                      NEW
                    </span>
                  )}
                </span>
      ...
      {!isFullHeight && (
        <div className="mt-auto border-t border-gray-100 bg-white p-3">
          <a href={category.viewAllUrl || '#'} className="flex items-center justify-center gap-1.5 w-full py-2.5 rounded text-white bg-[#104ba6] hover:bg-[#0b3b85] font-semibold text-[14.5px] transition-all group shadow-sm">
